	...
	return &framework.Path{
		...
		Fields: map[string]*framework.FieldSchema{
			...
			"role_trn": {
				...
				Description: `TRN of the role to be assumed. If provided, inline_policies and
...
Example: "name:VolcReadOnlyAccess,type:System".`,
			},
			"ttl": {
...
func (b *backend) operationRoleCreateUpdate(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	if raw, ok := data.GetOk("role_trn"); ok {
		role.RoleTRN = raw.(string)
	}
	...
	if role.Type() == roleTypeSTS {
		if len(role.RemotePolicies) > 0 {
			return nil, errors.New("remote_policies must be blank when a role_trn is present")
		}
		if len(role.InlinePolicies) > 0 {
			return nil, errors.New("inline_policies must be blank when a role_trn is present")
		}
	} else if len(role.InlinePolicies)+len(role.RemotePolicies) == 0 {
		return nil, errors.New("must include a role_trn, or at least one of inline_policies or remote_policies")
	}
	...
	if role.Type() == roleTypeSTS && (role.TTL > 0 || role.MaxTTL > 0) {
		resp.AddWarning("role_trn is set so ttl and max_ttl will be ignored because they're not editable on STS tokens")
	}
...
If you supply a role TRN, that role must have been created to allow trusted actors,
...
backend is mounted at "volcengine" and you create a role at "volcengine/roles/deploy",
then a user could request access credentials at "volcengine/creds/deploy".
`
